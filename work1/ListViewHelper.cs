...
namespace BitMainMinerTool
{
    ...
    public class ListViewColumnSorter : System.Collections.IComparer
    {
        ...
        public int Compare(object x, object y)
        {
            ...
            else if (float.TryParse(xText, out xFloat) && float.TryParse(yText, out yFloat))
            {
        ...
        {
            return System.Text.RegularExpressions.Regex.Match(ip, @"((?:(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d)))\.){3}(?:25[0-5]|2[0-4]\d|((1\d{2})|([1-9]?\d))))").Success;
        }
